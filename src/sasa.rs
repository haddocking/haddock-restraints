...
pub fn calculate_sasa(pdbtbx_struct: pdbtbx::PDB) -> Vec<ExtendedRes> {
    // ================================================================================
    ...
    for residue in pdbtbx_struct.residues() {
        let mut sasa_bb = 0.0;
        let mut sasa_sc = 0.0;
        ...
        let extended_residue = ExtendedRes::new(residue.clone(), sasa_bb, sasa_sc, 0.0, 0.0);
        extended_residues.push(extended_residue);
    }

    // Calculate the relative SASA of each residue, based on `asa::REL_ASA`
    for (e_res, res) in extended_residues.iter().zip(pdbtbx_struct.residues_mut()) {
